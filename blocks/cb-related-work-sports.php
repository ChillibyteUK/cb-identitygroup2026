<?php
/**
 * Block template for CB Related Work SPORTS.
 *
 * Same as CB Related Work, but always filtered to the "sports" theme
...
// Get service terms from current post. Theme is always 'Sports' for this block.
$services   = wp_get_post_terms( get_the_ID(), 'service' );
$sports_theme = get_term_by( 'slug', 'sports', 'theme' );
...
// Build theme ID for the Sports theme.
$theme_id = null;
if ( $sports_theme && ! is_wp_error( $sports_theme ) ) {
	$theme_id = $sports_theme->term_id;
}

// Run query if theme_id is found (this block always needs the Sports theme).
if ( $theme_id ) {
	$posts = array();

	// 1. Get up to 4 posts where Yoast primary service matches AND theme is Sports (only if service exists).
	if ( $service_id ) {
	...
	// 2. If less than 4, fill with posts assigned to the Sports theme (and service if available).
	if ( count( $posts ) < 4 ) {
