<?php
/**
 * Block template for CB Recent News — coda's own real design.
 *
 * Identical to identity's real version except the default (untagged)
 * case's colours/title — see cb-recent-news-identity.php. Kept as a
 * separate file matching each site's real source exactly, same as
 * index-identity.php/index-coda.php and single-identity.php/single-coda.php.
 *
...
switch ( $blog_type ) {
	...
	default:
		...
		$arrow         = '/img/arrow-bk.svg'; // coda's own real source references the non-existent "arrow-nbk.svg" — using the real, existing asset instead.
		$block_title   = 'RECENT INSIGHTS';
		break;
}

?>
<section id="<?php echo esc_attr( $block_id ); ?>" class="cb-latest-insights cb-recent-news <?= esc_attr( $background ); ?> has-lime-1000-border-top">
